...
2. Create a new directory with a file like [example.ts](./example.ts)
...
Now open a terminal in the directory and run:
```bash
npm i -g yarn
...
See [index.ts](src/index.ts) for available options.
...
See [example.ts](./example.ts) for example of features
...
First install [pm2](https://github.com/Unitech/pm2). (`npm i -g pm2`) Then copy [instabot.yml](./instabot.yml) into the same dir as `example.ts` and run:
